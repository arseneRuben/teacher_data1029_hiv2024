...
DROP PROCEDURE IF EXISTS productBuyBy;
DELIMITER |                                                      
CREATE PROCEDURE productBuyBy(IN v_fullName Varchar(30), i int)  
BEGIN
	IF i>0 THEN
		...
		WHERE u.full_name = v_fullName ;  
    ELSE 
	    SELECT "impossible ";
	END IF;
END |
DELIMITER ;
CALL productBuyBy('Oumar Moussa', 3);

DROP PROCEDURE IF EXISTS insertFakeProduct;
DELIMITER |                                                      
CREATE PROCEDURE insertFakeProduct(IN taille int, i int )
BEGIN 
	SET i = 0;
	WHILE( i < taille ) DO
        SET i = i+1;
		INSERT INTO `product` ( `name`, `supplier_id`, `min_qty`, `price`) 
        VALUES ( concat('produit',i),    (i%2)+1, i, 13.40);
    END WHILE;
END |
DELIMITER ;

CALL insertFakeProduct(5, 0);
