...
-- Liste des utilisateurs selon leur role
SELECT full_name AS 'Nom complet' , name AS 'Role' 
FROM user JOIN role ON user.role_id = role.id
ORDER BY name;

-- Noms et quantités totales par produits achetés par Oumar Moussa avec numero de facture
SELECT name AS 'Designation'  , SUM(quantity) AS 'Quantites' , invoice.id AS "Facture"
FROM product JOIN orderline ON product.id = orderline.product_id JOIN orders ON orders.id = orderline.order_id JOIN invoice ON invoice.order_id = orders.id JOIN user ON  invoice.customer_id = user.id && user.full_name = "Oumar Moussa"
GROUP BY product.id;

-- Dernier utilisateur a se deconnecter
SELECT full_name
FROM connection_history JOIN user ON connection_history.user_id = user.id
WHERE logout_date = (SELECT MAX(logout_date) FROM connection_history);

-- Produit livre par le fournisseur base a Monction

CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ1 AS
                    ...
                    DELIMITER //
                    CREATE PROCEDURE V_STUDENT_MARK_SEQ_CREATION( IN seq_order int, IN seq_id INT, IN room_id int, IN student_id int) 
                    BEGIN  
                      CASE seq_order  
                            WHEN 1 THEN             
                                      CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ1 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHEN std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                            WHEN 2 THEN             
                                      CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ2 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHEN std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                            WHEN 3 THEN             
                                      CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ3 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHEN std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                            WHEN 4 THEN             
                                      CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ4 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHEN std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                            WHEN 5 THEN             
                                      CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ5 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHEN std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                            WHEN 6 THEN            
                                      CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ6 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHEN std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                            WHEN 7 THEN            
                                      CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ7 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHEN std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                            WHEN 8 THEN            
                                      CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ8 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHEN std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                      END;
                   END //
                    ...
                    BEGIN  
                        CREATE OR REPLACE VIEW V_STUDENT_MARK_SEQ8 AS                     
                                        SELECT DISTINCT  eval.id as eval,crs.id as crs, room.id as room,  teach.full_name as teacher    , modu.id as module,m.value as value, m.weight as weight                     
                                        FROM mark  m   JOIN  student    std     ON  m.student_id        =   std.id JOIN  evaluation eval    ON  m.evaluation_id     =   eval.id                     JOIN  class_room room    ON   eval.class_room_id     =   roomteacher_id  =   teach.id                     JOIN  module     modu    ON  modu.id       =   crs.module_id                     JOIN  sequence   seq     ON  seq.id     =   eval.sequence_id                     
                                        WHERE std.id = student_id AND eval.sequence_id = seq_id   ORDER BY crs.id;
                    END //
                    DELIMITER ;
